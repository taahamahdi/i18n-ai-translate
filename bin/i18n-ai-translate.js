...
var require_src = __commonJS({
  "node_modules/iso-639-1/src/index.js"(exports2, module2) {
    ...
    module2.exports = class ISO63913 {
      static getLanguages(codes = []) {
        return codes.map(
          (code) => ISO63913.validate(code) ? Object.assign({}, LANGUAGES[code]) : { code, name: "", nativeName: "" }
        );
      }
      static getName(code) {
        return ISO63913.validate(code) ? LANGUAGES_LIST[code].name : "";
      }
      static getAllNames() {
        return LANGUAGE_NAMES.slice();
      }
      static getNativeName(code) {
        return ISO63913.validate(code) ? LANGUAGES_LIST[code].nativeName : "";
      }
...
// src/chat_interface/chatgpt.ts
var ChatGPT = class extends ChatInterface {
  model;
  chatParams;
  history;
  rateLimiter;
  constructor(model, rateLimiter) {
    super();
    this.model = model;
...
// src/chat_interface/gemini.ts
var Gemini = class extends ChatInterface {
  model;
  chat;
  history;
  params;
...
var verify = async (chat, verificationPromptText) => {
  ...
  try {
    ...
    console.error(`Failed to verify: ${e2}`);
  }
  return verification;
};
...
async function translate(options) {
  ...
  for (let i2 = allKeys.length - 1; i2 > 0; i2--) {
  ...
  for (let i2 = 0; i2 < Object.keys(flatInput).length; i2 += batchSize) {
    ...
    for (let j2 = 0; j2 < keys.length; j2++) {
      output[keys[j2]] = generatedTranslation.split("\n")[j2].slice(1, -1);
      if (options.verbose)
        console.log(
          `${keys[j2]}:
${flatInput[keys[j2]]}
...
var translateFileDiff = async (options) => {
  ...
  for (const outputPath of outputPaths) {
    ...
    try {
      ...
      console.error(`Invalid output JSON: ${e2}`);
    }
  }
  try {
...
var translateDirectoryDiff = async (options) => {
  ...
  for (const sourceFilePath of sourceFilePathsBefore) {
    ...
    for (const key in flatJSON) {
      if (Object.prototype.hasOwnProperty.call(flatJSON, key)) {
        inputJSONBefore[getTranslationDirectoryKey(
          sourceFilePath,
          key,
...
program.command("translate").requiredOption(
  ...
  if (options.outputLanguages) {
    ...
    for (const languageCode of getAllLanguageCodes()) {
      ...
      if (options.verbose) {
        console.log(
          `Translating ${i2}/${getAllLanguageCodes().length} languages...`
        );
      }
      const output = options.input.replace(
        getLanguageCodeFromFilename(options.input),
